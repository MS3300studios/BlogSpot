...
/*
    old logic for creating new lastReadMessages

    LastReadMessage.findOne({userId: req.userData.userId, conversationId: req.body.conversationId}).exec().then((err, document) => {
        if(err) console.log(err)
        else{
            if(!document){
                //document does not exist, checking if user belongs to conversation, if yes, creating last read message
                Conversation.findById(req.body.conversationId).then(conversation => {
                    let isParticipant = false;
                    conversation.participants.forEach(participant => {
                        if(participant.userId === req.userData.userId) isParticipant = true;
                    ...
                    if(isParticipant === true){
                        ...
                        lastReadMessage.save().then(lrmsg => {
                            res.json({
                                lastReadMessage: lrmsg
                            })
                        });
                    }
                    else{
                ...
                //updating lastReadMessage:
                // LastReadMessage.findByIdAndUpdate(req.body.conversationId).exec().then()
            }
        }
    })
